...
def process_documents(uploaded_files):
    try:
        ...
        for i, uploaded_file in enumerate(uploaded_files):
            ...
            if not chunks:
                st.sidebar.warning(f"Could not chunk {uploaded_file.name}")
                continue
        ...
        if all_chunks:
            ...
            st.sidebar.error("No chunks to process")
...
def handle_user_query(query):
    if not st.session_state.vectorstore:
        st.error("Please process documents before asking.")
        return
    ...
    try:
        ...
        response, context, _ = query_with_full_context(query, st.session_state.vectorstore, k=k, temperature=temp)
